...
// PATCHED: every ref below is always initialised non-null (`useRef(ctx)`,
// never `useRef(null)`), but upstream typed the loop's params as the
// read-only `RefObject<T>` (`.current: T | null` in this app's React 18
// types), so every `.current` read here failed strict-null-check. Retyped
// to `MutableRefObject<T>` (`.current: T`, no cast needed at the call site)
// instead of asserting `!` at ~40 read sites. `@dither-kit/cli update` will
// revert it — see README.md.
